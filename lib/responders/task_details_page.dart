...
class ResponderTaskDetailsPage extends StatelessWidget {
  final Map<String, dynamic> taskData;

  const ResponderTaskDetailsPage({
    Key? key,
  ...
  // Remove placeholder tokens and Plus Codes from a comma-separated address string.
  // If everything is removed, return exactly 'Not provided'.
  static String _cleanAddressDisplay(String? raw) {
    ...
    final parts = raw
        ...
        .where((s) {
          ...
          if (s.contains('+') && s.length <= 12) return false; // Plus code-like
          return true;
        })
        .toList();
  ...
  Future<void> _openSharedLocation(BuildContext context) async {
    try {
      ...
      if (respondersSnap.exists && respondersSnap.value is Map) {
        final rootMap = respondersSnap.value as Map;
  ...
  Widget _buildHeaderRich(
    ...
    Widget chip({
      ...
      IconData? icon,
      Color color = const Color.fromARGB(51, 255, 255, 255), // white 20%
          ...
          children: [
            Row(
              children: [
                ...
                const Spacer(),
                TextButton.icon(
                  onPressed: () => _respondToCall(context),
                  icon: const Icon(Icons.play_arrow_rounded, color: Colors.white),
                  label: const Text('Respond', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          _buildHeaderRich(context, callerName, photoUrl, gender, contact, address, birthDate, disability, medical),
          Expanded(
                ...
                children: [
                  ...
                  _SectionCard(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        // Address (prefer reverse geocoded from coordinates if available)
  ...
  Future<void> _respondToCall(BuildContext context) async {
    try {
      ...
      final assignedRef = root.child('$basePath/Assigned/$callId');
      final inProgressRef = root.child('$basePath/InProgress/$callId');

      // If already In Progress, bail early
      final inProgSnap = await inProgressRef.get();
      if (inProgSnap.exists) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('This task is already In Progress.')),
        );
        return;
      }

      final snap = await assignedRef.get();
      if (!snap.exists) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Assigned task not found.')),
        );
        return;
      }
      ...
      data['inProgressAt'] = DateTime.now().toIso8601String();

      await root.update({
        '$basePath/InProgress/$callId': data,
        '$basePath/Assigned/$callId': null,
      });
